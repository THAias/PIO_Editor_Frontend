...
import "../styles/basic/pdfexport.scss";
import { convertToObject } from "./DocumentService";
 ...
 */
export const getPioHTMLElement = (): HTMLElement => {
    const tabHeaders = Array.from(document.getElementsByClassName("ant-tabs-tab-btn"));
    ...
    Array.from(contentTabs).forEach((tab: Element, index: number) => {
        const newTab = tab.cloneNode(true) as HTMLElement;
        const tabHeader: HTMLDivElement = document.createElement("div");
        tabHeader.classList.add("tab-header-name");
        tabHeader.innerText = tabHeaders.at(index)?.textContent ?? "";
        ...
        pdfElement.appendChild(tabHeader);
 ...
 */
export const getDocumentPaths = (): string[] | undefined => {
 ...
 */
export const getDocuments = async (allDocumentPaths: string[]): Promise<IUploadDocumentObject[]> => {
 ...
 */
export const addPdfOrImageDocumentsToPioPDF = async (
    pioPdf: PDFDocument,
 ...
 */
export const addFixedElementsToPDF = (
    pioPdf: PDFDocument,
 ...
 */
export const addTextDocumentsAndTableOfContentToHtmlElement = (
    documents: IUploadDocumentObject[],
...
export const exportPioPDF = async (sendingOrganization: string): Promise<void> => {
    ...
    try {
        ...
        await drawDOM(pioPdfElement, pdfOptions)
            .then((group) => {
        ...
        const pdfElement = document.getElementsByClassName("pdf-override");
        if (pdfElement?.[0]) pdfElement[0].remove();
        console.error("An error occurred during pdf export:" + error);
    }
};
